...
export function applyReward(reward = {}, ctx = {}) {
  // `items` holds drops KEPT in the inventory; `scrapped` counts drops that were
  // auto-salvaged instead (their scrap/essence is folded into `resources`).
  const summary = { xp: 0, levelsGained: 0, resources: {}, items: [], scrapped: 0, source: ctx.source };

  const addRes = (map) => {
    for (const [k, v] of Object.entries(map || {})) summary.resources[k] = (summary.resources[k] || 0) + v;
  };
  ...
  if (Array.isArray(reward.items)) {
    for (const item of reward.items) {
      ...
      if (item.roll === 'equipment') {
        ...
        if (shouldAutoScrap(gen)) {
          // Auto-scrap: never enters the inventory; yields resources instead.
